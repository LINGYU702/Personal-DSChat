...
- Sidebar（新对话/会话列表/设置入口按钮）、Topbar（模型选择器）、ChatView（欢迎视图 + 消息列表 + ChatInput）
...
- 设置页重构（FR-17，settings.md 第 1~3 节）：双栏类别布局（左类别导航 + 右条目，尺寸与库管理弹窗一致，分隔线可拖拽）+ 数据类别（导出/导入/导入对话…/清除本地数据）+ 边栏「设置与偏好」按钮直接打开设置
