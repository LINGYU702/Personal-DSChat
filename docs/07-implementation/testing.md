...
- [ ] FR-15 设置对话框「数据」类别「导出数据」→ 下载 `deepseek-chat-backup-*.json`；打开文件确认含 format/version 字段、全部会话与自定义 Prompt（无内置条目）
...
- [ ] 设置对话框「数据」类别「导入对话…」导入本应用单会话 JSON → 确认框（标题/消息数）→ 会话出现在边栏可查看；再次导入同文件 → 「跳过」提示，本地不被覆盖
